#!/usr/bin/env python3
"""CIFAR-100 AEES-minus-control paired effect plot (six noise/optimizer cells).

Two panels (AdamW, SGD+M), each showing the paired mean effect
    AEES - control
in percentage points of peak validation accuracy, for three noise settings
(Asym. 20%, Sym. 20%, Sym. 40%) and three controls:

    1. retrospectively best fixed LR multiplier (m in {0.5, 1.0, 2.0}),
       selected per noise/optimizer cell by mean peak validation accuracy
       across seeds -- exactly the selection in
       scripts/tables/make_cifar_noisy_fixed_lr_ablation_tables.py
       (``best_fixed_for``);
    2. uniform-random open-loop control;
    3. frequency-matched open-loop control.

All three comparisons reuse the exact same paired seed-level bootstrap
machinery (``scripts.tables._paired_stats.compare_paired``: 10,000-resample
percentile bootstrap over the five paired seed differences, 95% CI) already
used by the existing supplementary tables. No values are hard-coded from the
manuscript -- everything here is recomputed from the result files under
``archived_results/cifar_noisy`` (AEES + fixed-multiplier runs) and
``archived_results/aees-runpod-results`` (uniform-random + frequency-matched
open-loop control runs).

Validation: before plotting, the six best-fixed and six UR/FM effects are
compared against the known-good manuscript values and the script aborts
(non-zero exit, no figure written) if any of them drift outside tolerance.

CLI:
    uv run python -m scripts.plots.cifar.plot_cifar_aees_vs_control_effect

Outputs (on success):
    cifar_aees_vs_control_effect.pdf
    cifar_aees_vs_control_effect.png
    cifar_aees_vs_control_effect.summary.txt

Outputs (on failure):
    cifar_aees_vs_control_effect.MISSING.md
"""
...
NAME = "cifar_aees_vs_control_effect"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="Directory in which to write the cifar_aees_vs_control_effect outputs.",
    )
